...
- Liveness: every client is subscribed to `presence`, so the broker echoes its own heartbeats back. `Radio.tick()` calls `reconnect()` when the echo stops for `ECHO_TIMEOUT_MS` (or a `wake()` probe goes unanswered), which force-ends the client and opens a new one with the same client id so the broker kicks the stale session. Don't subscribe with "no local" semantics or the watchdog breaks. Handlers registered in `openClient()` are guarded with `this.client === client` because a replaced client keeps emitting until its stream dies.
...
### Background operation (screen off / app in background)
Must keep working on both Android Chrome and iOS Safari; the pieces are deliberately redundant:
- `AudioEngine.init()` starts a looping **silent WAV in an `<audio>` element** (`startKeepAlive`, `KEEPALIVE_S` ≥ 5 s: shorter tracks get no media controls in Chrome). iOS suspends Web Audio in the background unless a media element is playing; it also makes the page ignore the iPhone silent switch and gives it a lock-screen card. A `MediaStream`-fed element would not work: Chrome never makes it controllable. `standby()` (power off) pauses it and suspends the context; `resume()` restarts both and is called on `visibilitychange`, `resume`, `online` and the context's `statechange` (iOS "interrupted").
- `navigator.audioSession.type` (WebKit only) is `"playback"` while the radio is on and `"play-and-record"` only while the mic is open: with a `playback` override iOS would mute capture.
- `app.js` publishes Media Session metadata (channel + state) and maps `play`/`pause`/`stop` to power on/off; it also holds a shared Web Lock while on (Chrome doesn't freeze pages holding one). On `hidden` it releases PTT and closes the mic (iOS mutes it in the background); on wake it calls `Radio.wake()`.
- **All `radio.js` timers go through `timers.js`** (a Blob Web Worker; falls back to window timers), because hidden pages get main-thread timers throttled to once a minute. UI-only timers in `app.js`/`audio.js` stay native. mqtt.js's own keepalive already uses a worker timer (`timerVariant: "auto"`); its reconnect timer does not, which is acceptable.
- Transmitting from the background is not a goal (PTT needs the screen). Nothing here can be tested from Linux Chrome beyond the desktop behaviour; changes to this area need a check on real phones.
